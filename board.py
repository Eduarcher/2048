...
class Board:
    ...
    def check_board(self):
        for i in range(self.lx):
            for j in range(self.ly):
                if 0 in [i, j] or self.lx-1 in [i, j] or self.lx+1 in [i, j]:
                    if self.tiles[i][j] == 0:
                        return True
                    if i == 0:
                        if self.tiles[i + 1][j] == self.tiles[i][j]:
                            return True
                    elif i == self.lx - 1:
                        if self.tiles[i - 1][j] == self.tiles[i][j]:
                            return True
                    ...
                    if self.tiles[i - 1][j] == self.tiles[i][j] or self.tiles[i + 1][j] == self.tiles[i][j]:
                        return True
                    if self.tiles[i][j - 1] == self.tiles[i][j] or self.tiles[i][j + 1] == self.tiles[i][j]:
                        return True
        return False
    ...
    def b_up(self):
        ...
        while i < self.lx:
            ...
            merge -= 1
            i += 1
        if updated is False and self.spare_play:
            updated = True
            self.spare_play = False
